...
string datafile_probe="/Users/suyoupeng/Downloads/rcbkdipole-master/data/proton/mve.dat";
...
void drawdNdydpTPi0(){
    ...
    string datafile3 = "/Users/suyoupeng/Downloads/rcbkdipole-master/src/ALICEdata.txt"; 

    string datafile4 = "/Users/suyoupeng/Downloads/rcbkdipole-master/src/MVe_Pi0dNdydpt.txt"; 
    ...
    while (inputFile2 >> k >> xg)
    {
        graph2->SetPoint(i, k, xg*16.36*16.36*1/70*2*TMath::Pi());//**16.36*16.36*1/70*2*TMath::Pi()
        i++;
    }
    inputFile2.close();
    while (inputFile3 >> k >> xg)
    ...
    while (inputFile4 >> k >> xg)
    {
        graph4->SetPoint(i, k, xg*16.36*16.36*1/70*2*TMath::Pi());
        i++;
    }
    inputFile4.close();
    ...
    graph4->SetLineColorAlpha(kRed,1); 
    graph4->SetLineStyle(2);      
    graph4->SetLineWidth(6); 
    ...
    legend->AddEntry(graph3, "ALICE data,y = 0 ", "ap");  
    legend->AddEntry(graph4, "(x (#sigma_{0}/2)^{2}x 1/#sigma_{inel} x 2#pi ) k_{T} & kkp:My code simulation:MV^{e},adjoint rep", "l");  
...
void Dipole_UGD_2D() {
    ...
    for (double Y = Y_min; Y <= Y_max; Y += Y_step) {
        ...
        for (double q = 0; q <= q_max; q += q_step) {
            double ugd = Dipole_UGD(q, x, q*q, 1); 
...
void Dipole_UGD_x(){
    ...
    g2d->SetMarkerColor(kBlue); 

    g2d1->SetMarkerStyle(5); 
    g2d1->SetMarkerColor(kRed); 
...
//mode:KTFTACT_PT
void calculate_HadronMultiplicity_dyd2pt_ktfact(){
    ...
    string outputFileName1 = "/Users/suyoupeng/Downloads/rcbkdipole-master/src/MVe_Pi0dNdydpt.txt";
    ofstream outputFile1(outputFileName1);
