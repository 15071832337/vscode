...
LOCAL_SAVE_DIR="/home/suyoupeng/Downloads/lambdaspincorefile/Analysiscode/LHC23_pass4_thin_690954_AnalysisResult"
...
process_hy_number() {
    ...
    for SUB_FOLDER in "${SUB_FOLDERS[@]}"; do
        ALIEN_FILE_PATH="${AOD_PATH}/${SUB_FOLDER}/AnalysisResults.root"
        LOCAL_FILE_PATH="${LOCAL_SAVE_DIR}/AnalysisResults_${HY_NUM}_${SUB_FOLDER}.root"

        # 1. 检查AnalysisResults.root文件是否存在
        if ! check_alien_file_exists "${ALIEN_FILE_PATH}"; then
            echo "ℹ️  hy_${HY_NUM}_${SUB_FOLDER} AnalysisResults.root不存在，跳过"
